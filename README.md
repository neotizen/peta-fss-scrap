...
Python `3.10+` 기준으로 동작합니다. Python `3.10`에서는 `tomli`가 함께 설치되어 `config.toml`을 읽습니다.
...
py -3 -m venv .venv
.venv\Scripts\python.exe -m pip install -r requirements.txt
.venv\Scripts\python.exe -m uvicorn web_app:app --host 0.0.0.0 --port 8080
```
...
임시 작업 폴더는 각 OS의 기본 임시 디렉터리를 사용하므로 Windows에서도 `/tmp` 같은 고정 Unix 경로에 의존하지 않습니다.
...
- 조회 시작일
- 조회 종료일
- 저장 하위 폴더 (비워두면 `~/Documents`)
- 첨부 문서 포함 여부 (`y/n`, 기본값 `n`)
- 추가 분할 저장 크기 (`MB`, 기본값 `199`, `n` 입력 시 추가 분할 안 함)

저장 파일명은 실제로 수집된 공시의 접수일자 범위를 기준으로 `금감원공시-{회사명}-{최초접수일YYMMDD}-{최종접수일YYMMDD}.pdf` 형식으로 자동 생성됩니다.
원본 PDF는 항상 먼저 저장되고, 분할을 켜면 같은 폴더에 `...-part001.pdf`, `...-part002.pdf`가 추가로 생성됩니다.
...
.venv/bin/python build_disclosure_pdf.py --split-max-mb 199
.venv/bin/python build_disclosure_pdf.py --no-split
...
.venv\Scripts\python.exe build_disclosure_pdf.py --split-max-mb 199
.venv\Scripts\python.exe build_disclosure_pdf.py --no-split
...
생성된 PDF를 용량 기준으로 나눠야 할 때만 별도 스크립트 [`split_pdf_by_size.py`](/Users/neotizen/peta-fss-scrap/split_pdf_by_size.py)를 실행하면 됩니다. 기본 분할 기준은 `199MB`이며, 결과 파일은 원본 PDF와 같은 폴더에 `...-part001.pdf`, `...-part002.pdf` 형식으로 저장됩니다.

```bash
.venv/bin/python split_pdf_by_size.py /path/to/output.pdf
.venv/bin/python split_pdf_by_size.py /path/to/output.pdf --max-mb 150
.venv/bin/python split_pdf_by_size.py /path/to/output.pdf --max-mb 150 --output-dir /path/to/splits
```

```cmd
.venv\Scripts\python.exe split_pdf_by_size.py C:\path\to\output.pdf
.venv\Scripts\python.exe split_pdf_by_size.py C:\path\to\output.pdf --max-mb 150
.venv\Scripts\python.exe split_pdf_by_size.py C:\path\to\output.pdf --max-mb 150 --output-dir C:\path\to\splits
```
...
4. 모든 페이지 왼쪽 상단에 `회사명(종목코드)-공시일자(yy.mm.dd)-공시명` 헤더를 넣어 원본 PDF를 저장합니다.
5. 분할 기준을 지정하면 원본 PDF를 유지한 채 추가 분할 파일을 생성합니다.
6. 웹 모드에서는 같은 로직을 서버에서 실행한 뒤 생성된 PDF를 바로 응답으로 내려줍니다.
